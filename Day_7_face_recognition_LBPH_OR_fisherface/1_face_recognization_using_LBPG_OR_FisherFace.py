...
face_width = 150
face_Height = 150
img_folder_path = r"C:\Users\chira\PycharmProjects\AI_and_ML_Hackathon\Day_7_face_recognition_LBPH_OR_fisherface\data"

#######################  Setting  ##############################

print("Training....")
(images,labels,names) = ([], [], {})    # ie. 1.png-0-elon

for (subDirs, Dir, Files) in os.walk(img_folder_path):
    id = 0
    for subDir in Dir:
        names[id] = subDir
...
(images, labels) = [numpy.array(lst) for lst in [images, labels]]   # converting to numpy arrays
...
while True:
    ...
    faces_in_gray = haar_cascade_facial_classifier.detectMultiScale(gray_img, scale, neighbour)
    for (x,y,w,h) in faces_in_gray:
